...
USE [ROWCA]
GO
/****** Object:  StoredProcedure [dbo].[GetAllTasksDataReport]    Script Date: 11/12/2013 2:39:13 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
	...
	SELECT * INTO #emergency FROM dbo.fn_ParseCSVString (@emergencyIds, ',')
	SELECT * INTO #months FROM dbo.fn_ParseCSVString (@monthIds, ',')
	SELECT * INTO #passedLocations FROM dbo.fn_ParseCSVString (@locationIds, ',')
	SELECT * INTO #clusters FROM dbo.fn_ParseCSVString (@clusterIds, ',')
	SELECT * INTO #orgs FROM dbo.fn_ParseCSVString (@orgIds, ',')
	SELECT * INTO #orgTypes FROM dbo.fn_ParseCSVString (@orgTypeIds, ',')
	...
	;WITH allChildLocations AS
	(
		SELECT LocationId, LocationParentId
		FROM Locations
		WHERE LocationId IN (SELECT * FROM #passedLocations)
		UNION ALL
		SELECT l.LocationId, l.LocationParentId FROM Locations l join allChildLocations c on l.LocationParentId = c.LocationId
	)

	SELECT LocationId INTO #locations FROM allChildLocations
	
	DROP TABLE #passedLocations
...
USE [ROWCA]
GO
/****** Object:  StoredProcedure [dbo].[GetAllTasksDataReport1]    Script Date: 11/12/2013 4:54:38 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
USE [ROWCA]
GO
/****** Object:  StoredProcedure [dbo].[GetOrganizationOffices]    Script Date: 11/12/2013 5:30:33 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
-- Description:	Get Offices On Organization
-- =============================================
ALTER PROCEDURE [dbo].[GetOrganizationOffices]
...
BEGIN
	...
	-- Get all childs of the parent location
	;WITH ChildLocations AS
	(
		SELECT l.LocationID, l.LocationName, l.LocationParentId
		FROM Locations l
		WHERE LocationID = @parentLocationId
		
		UNION ALL
		
		SELECT ll.LocationID, ll.LocationName, ll.LocationParentId
	...
	FROM	Offices f JOIN Organizations o ON f.OrganizationId = o.OrganizationId
	WHERE	(f.OrganizationId = @organizationId OR @organizationId IS NULL)
	AND		f.LocationId IN (SELECT LocationId FROM ChildLocations)
	ORDER BY f.OfficeName
END
...
USE [ROWCA]
GO
/****** Object:  StoredProcedure [dbo].[GetAllTasksDataReport1]    Script Date: 11/12/2013 8:06:55 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
