...
{
    public partial class ctlOPSAdmin1Targets : System.Web.UI.UserControl
    {
        ...
        private void SaveAdmin1Targets(Repeater rptAdmin1, int indicatorId, int countryId)
        {
            ...
            foreach (RepeaterItem item in rptAdmin1.Items)
            {
                if (item.ItemType == ListItemType.Item || item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (admin1Id > 0)
                    {
                        DBContext.Update("InsertOPSReportDetails_Admin1", new object[] { OPSReportId, indicatorId, countryId, admin1Id, 
